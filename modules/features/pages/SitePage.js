...
function genMockResponse(devId, modeId){
  ...
  if (mid===1){
    ...
    if (!st.items || prob(0.10)) {
      const n = Math.floor(rnd(0,5)); // 0~4
      st.items = Array.from({length:n},(_,i)=>({ name:`倾角${i+1}#`, deg: rnd(0,1), batt: rnd(70,100), alarmOn: Math.random()>.2, sirenOn: Math.random()>.2 }));
    }
    st.items.forEach(it=>{
  ...
  if (mid===2){
    ...
    if (!st.list || prob(0.10)) {
      const nDisp = Math.floor(rnd(0,4)); // 0~3 位移
      const nTilt = Math.floor(rnd(0,4)); // 0~3 倾角
      const list = [];
      for (let i=0;i<nDisp;i++) list.push({ type:'位移', badge: Math.floor(rnd(10,99)), batt: rnd(70,100), sirenOn: Math.random()>.3, value: rnd(0,0.012) });
      for (let i=0;i<nTilt;i++) list.push({ type:'倾角', badge: Math.floor(rnd(60,99)), batt: rnd(70,100), sirenOn: Math.random()>.3, valueDeg: rnd(0,0.30) });
      st.list = list;
    }
    st.list.forEach(it=>{
    ...
    return {
      items: st.list.map(it=> it.type==='位移'
